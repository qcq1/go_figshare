...
```go
auth := context.WithValue(context.Background(), go_figshare.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, go_figshare.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
